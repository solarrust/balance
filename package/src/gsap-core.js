...
let _config = {
  ...
  _addGlobal = (name, obj) =>
    (name &&
      (_globals[name] = obj) &&
      _installScope && (_installScope[name] = obj)) ||
    _globals,
  _emptyFunc = () => 0,
  _reservedProps = {},
  _lazyTweens = [],
  ...
  toArray = (value, leaveStrings) =>
    _isString(value) && !leaveStrings && (_coreInitted || !_wake())
      ...
      ? [value]
      : [],
  shuffle = a => a.sort(() => 0.5 - Math.random()), // alternative that's a bit faster and more reliably diverse but bigger:   for (let j, v, i = a.length; i; j = ~~(Math.random() * i), v = a[--i], a[i] = a[j], a[j] = v); return a;
  ...
  mapRange = (inMin, inMax, outMin, outMax, value) => {
    ...
    return _conditionalReturn(
      value,
      value => outMin + ((value - inMin) / inRange) * outRange
    );
  },
    ...
    if (parent && parent.smoothChildTiming && this._ts) {
      ...
      this._start = _round(
        parent._time -
          (this._ts > 0
            ? totalTime / this._ts
            : ((this._dirty ? this.totalDuration() : this._tDur) - totalTime) /
              -this._ts)
      );
      _setEnd(this);
      if (!parent._dirty) {
...
const _gsap = {
  ...
  quickSetter(target, property, unit) {
    ...
    return Plugin
      ? setter
      : value =>
          setter(target, property, unit ? value + unit : value, cache, 1);
  },
  isTweening(targets) {
